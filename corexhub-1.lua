...
-- Wait for LocalPlayer + PlayerGui to be fully ready before loading UI
-- This prevents WindUI "attempt to index nil with 'Parent'" errors
local _lp = Players.LocalPlayer
if not _lp then
    Players:GetPropertyChangedSignal("LocalPlayer"):Wait()
    _lp = Players.LocalPlayer
end
if not _lp:FindFirstChild("PlayerGui") then
    _lp:WaitForChild("PlayerGui")
end
...
local function EnableAntiKick()
    ...
    local ok, err = pcall(function()
        ...
        if not mt then
            warn("[anti-kick] getrawmetatable not available on this executor")
            return
        end
        ...
        elseif setreadonly and newcclosure then
            pcall(function()
            ...
            warn("[anti-kick] Metamethod hooking not supported on this executor — skipping namecall hook")
        end

        if lp.Kick and hookfunction then
            pcall(function()
    ...
    if not ok then
        warn("[anti-kick] Hook setup failed: " .. tostring(err))
    end

    -- hide kick GUI (safe loop, no executor funcs needed)
    task.spawn(function()
...
local function DisableAntiKick()
    ...
    pcall(function()
        ...
        if restoreFuncs.oldKick and hookfunction then
            pcall(function()
...
-- ================================
-- ANTI MARTIAL ARTS ALL SKILL SYSTEM
-- ================================

-- STATE
local AntiMartialArtsEnabled = false
local AntiMartialArtsConnections = {}
local MartialArtsWatching = {}

-- Clean disconnect function
local function DisconnectAllMartialArts()
    for _, conn in ipairs(AntiMartialArtsConnections) do
        pcall(function() conn:Disconnect() end)
    end
    AntiMartialArtsConnections = {}
    MartialArtsWatching = {}
end

-- Enable Anti Martial Arts All Skill
local function EnableAntiMartialArts()
    if AntiMartialArtsEnabled then return end
    AntiMartialArtsEnabled = true
    ...
    if player.Character then loadChar(player.Character) end
    table.insert(AntiMartialArtsConnections, player.CharacterAdded:Connect(loadChar))

    ------------------------------------------------------
    -- Tween backward (same as Anti Flowing Water)
    ------------------------------------------------------
    local function tweenBackward(dist)
        if not hrp then return end
        local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Linear)
    ...
    ------------------------------------------------------
    -- Tween left (same as Anti Flash Strike)
    ------------------------------------------------------
    local function tweenLeft(dist)
        if not hrp then return end
        local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Linear)
    ...
    ------------------------------------------------------
    -- Animation ID groups
    -- Group A (Flowing Water logic): tweenBackward x2
    -- Group B (Flash Strike logic):  tweenLeft x1
    ------------------------------------------------------
    local MARTIAL_ARTS_GROUP_A = {
        ["rbxassetid://17857788598"] = true,
        ["rbxassetid://17799224866"] = true,
    }

    local MARTIAL_ARTS_GROUP_B = {
        ["rbxassetid://18179181663"] = true,
        ["rbxassetid://17838006839"] = true,
    }

    ------------------------------------------------------
    -- Watch a player for Martial Arts animations
    ------------------------------------------------------
    local function watchPlayer(plr)
        if MartialArtsWatching[plr] then return end
        MartialArtsWatching[plr] = true
        ...
        local function setup(char2)
            ...
            local conn = h.AnimationPlayed:Connect(function(track)
                ...
                if not (MARTIAL_ARTS_GROUP_A[animId] or MARTIAL_ARTS_GROUP_B[animId]) then return end

                task.spawn(function()
                    pcall(function()
                        if not (hrp and char2:FindFirstChild("HumanoidRootPart")) then return end
                        local dist = (hrp.Position - char2.HumanoidRootPart.Position).Magnitude
                        if dist > 50 then return end

                        if MARTIAL_ARTS_GROUP_A[animId] then
                            playMyAnimation()
                            tweenBackward(18)
                            task.wait(0.6)
                            tweenBackward(18)
                        elseif MARTIAL_ARTS_GROUP_B[animId] then
                            playMyAnimation()
                            tweenLeft(18)
                        end
                    end)
            ...
            table.insert(AntiMartialArtsConnections, conn)
        end
        ...
        -- Respawn handling
        table.insert(AntiMartialArtsConnections, plr.CharacterAdded:Connect(function(c)
            pcall(function() setup(c) end)
        end))
    end
    ...
    -- Watch new players
    ------------------------------------------------------
    table.insert(AntiMartialArtsConnections, Players.PlayerAdded:Connect(function(plr)
        if plr ~= player then watchPlayer(plr) end
    end))
end

-- Disable Anti Martial Arts All Skill
local function DisableAntiMartialArts()
    if not AntiMartialArtsEnabled then return end
    AntiMartialArtsEnabled = false
    DisconnectAllMartialArts()
end
...
-- ================================
AntiTab:Toggle({
    Title = "Anti Martial Arts All Skill",
    Desc = "Dodge all Martial Arts skills (50 studs range)",
    Value = false,
    Callback = function(state)
        if state then
            EnableAntiMartialArts()
        else
            DisableAntiMartialArts()
        end
    end
})
